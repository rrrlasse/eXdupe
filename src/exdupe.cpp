...
void parse_flags(void) {
    ...
    while (argc > i && argv.at(i).substr(0, 1) == L("-") && argv.at(i).substr(0, 2) != L("--") && argv.at(i) != L("-stdin") && argv.at(i) != L("-stdout")) {
        ...
        if (flags.length() > 2 && flags.substr(0, 2) == L("-u")) {
            ...
            size_t e = flags.find_first_not_of(L("-wfhuRrxqcDpiLzSksatgmv0123456789B"));
            if (e != string::npos) {
    ...
    // =================================================================================================
        ...
        if (diff_flag) {
           ...
           // ifile = FOPEN(full.c_str(), L("ab+"));
            ...
            memory_usage = read_header(ifile, full, BACKUP, &archive_id); // also inits hash_salt and sets
