    ...
    while (src <= last) {
        ...
        while (src <= upto) {
            ...
            if (src + SMALL_BLOCK - 1 <= upto) {
                match_s = dub(src, pay + (src - src_orig), (upto - src), SMALL_BLOCK, &ref_s);
                if (match_s) {
                    n = 1;
                    while (ref_s < pay && src + (n + 1) * SMALL_BLOCK - 1 <= upto) {
                        uint64_t ref_s0 = 0;
                        auto m = dub(src + n * SMALL_BLOCK, pay + (src + n * SMALL_BLOCK - src_orig), 1, SMALL_BLOCK, &ref_s0);
                        if (m == src + n * SMALL_BLOCK && ref_s0 == ref_s + n * SMALL_BLOCK) {
                            n++;
                        }
                        else {
                            break;
            ...
            if (n == 0) {
                ...
                dst += write_match(minimum(n * SMALL_BLOCK, upto - match_s + 1), ref_s, dst);
                src = match_s + n * SMALL_BLOCK;
            }
        }
