...
TEST_CASE("acl_roundtrip_verify") {
...
#ifdef _WIN32
    auto icacls_set = [&](const string &path, const string &perm) {
        string q = string("\"") + p(path) + "\"";
        bool isLink = is_symlink(s2w(path));
        string linkFlag = isLink ? "/L" : "";
        sys("icacls", q, "/inheritance:r");
        sys("icacls", q, "/grant:r", perm);
    };

    icacls_set(src + "/f1.txt", "Everyone:R");
    icacls_set(src + "/d1", "Everyone:F");
    icacls_set(src + "/d1/f2.txt", "Everyone:M");

#else
    chmod(p(src + "/f1.txt").c_str(), S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH); // 0644
    chmod(p(src + "/d1").c_str(), S_IRWXU);                                   // 0700
    chmod(p(src + "/d1/f2.txt").c_str(), S_IRUSR | S_IWUSR);                  // 0600
#endif
    ...
    auto read_icacls = [&](const string &path) -> string {
        string q = string("\"") + p(path) + "\"";
        string out = sys("icacls", "/L", q); // L = don't follow link

        // Keep only lines that contain ACL entries (contain '\\' or ':' or '(' )
        ...
        while (std::getline(iss, line)) {
            ...
            if (trimmed.rfind("Successfully processed", 0) == 0)
                continue;
            if (trimmed.rfind("processed file", 0) == 0)
                continue;
            if (trimmed.rfind("Failed processing", 0) == 0)
                continue;

            // icacls output typically is: "<path> <ACE...>". We need to compare ACEs only (not the path)
            size_t pos_paren = trimmed.find('(');
            string ace_part;
            if (pos_paren != string::npos) {
                size_t space_before = trimmed.rfind(' ', pos_paren);
                if (space_before != string::npos && space_before + 1 < trimmed.size()) {
                    ace_part = trimmed.substr(space_before + 1);
                } else {
                    size_t first_space = trimmed.find(' ');
                    ace_part = (first_space == string::npos) ? trimmed : trimmed.substr(first_space + 1);
                }
            } else {
                size_t first_space = trimmed.find(' ');
                ace_part = (first_space == string::npos) ? trimmed : trimmed.substr(first_space + 1);
            ...
            auto f2 = ace_part.find_first_not_of(" \r\n\t");
            if (f2 != string::npos) {
                auto l2 = ace_part.find_last_not_of(" \r\n\t");
                lines.push_back(ace_part.substr(f2, l2 - f2 + 1));
            }
        }
...
#ifdef _WIN32
        string s_acl = read_icacls(sitem);
        string d_acl = read_icacls(ditem);
