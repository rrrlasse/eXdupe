...
TEST_CASE("acl_roundtrip_verify") {
    ...
    auto set_acl = [](const std::string &path, const std::string &aceString) {
        ...
        if (colon == std::string::npos) {
            throw std::invalid_argument("ACE string format must be 'Principal:Rights'");
        }
    ...
    set_acl(src + "/f1.txt", "Everyone:ReadAndExecute");
    set_acl(src + "/d1", "Everyone:FullControl");
    set_acl(src + "/d1/f2.txt", "Everyone:Modify");
    set_acl(src + "/link_to_f1.txt", "Everyone:ChangePermissions");
    set_acl(src + "/link_to_d1.txt", "Everyone:Traverse");
...
// Needed by io.cpp
void abort(bool b, retvals ret, const std::string &s) { CHECK(false); };
void abort(bool b, retvals ret, const std::wstring &s) { CHECK(false); };
void abort(bool b, const CHR *fmt, ...) { CHECK(false); }
